...
module.exports = {
    exec: async ({ message, args }) => {
        ...
        const user_exp = existsSync(`./database/users/${message.author.id}.json`) ? require(`../../../_database/users/exp/${message.author.id}.json`) : { level: 0, exp: 0 };

        message.channel.send({
            embed: {
                author: { name: `Niveau de ${member.user.tag}`, icon_url: member.user.displayAvatarURL({ size: 2048, dynamic: true, format: "png" }) },
                fields: [
                    { title: "Niveau", value: user_exp.level, inline: true },
